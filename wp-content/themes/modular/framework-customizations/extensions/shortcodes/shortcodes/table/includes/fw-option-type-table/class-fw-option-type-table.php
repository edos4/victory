...
class Wellnesscenter_Option_Type_Table extends FW_Option_Type {
    ...
    protected function _enqueue_static($id, $option, $data) {
        ...
        wp_enqueue_style(
                'fw-option-' . $this->get_type() . '-default', $static_uri . 'css/default-styles.css', array(), fw()->theme->manifest->get_version()
        );
        wp_enqueue_style(
                'fw-option-' . $this->get_type() . '-extended', $static_uri . 'css/extended-styles.css', array(), fw()->theme->manifest->get_version()
        );
        wp_enqueue_script(
                'fw-option-' . $this->get_type(), $static_uri . 'js/scripts.js', array('jquery', 'fw-events', 'jquery-ui-sortable'), fw()->theme->manifest->get_version(), true
        );
        $localization_array = array(
            'msgEdit' => __('Edit', 'modular'),
    ...
    protected function _render($id, $option, $data) {
        ...
        if (!$table_shortcode) {
            trigger_error(
                    __('table-builder option type must be inside the table shortcode', 'modular'), E_USER_ERROR
            );
        }
    ...
    protected function _get_defaults() {
        return array(
            'header_options' => array(
                'table_purpose' => array(
                    'type' => 'select',
                    'label' => __('Table Styling', 'modular'),
                    'desc' => __('Choose the table styling options', 'modular'),
                    'choices' => array(
                        'pricing' => __('pricing table style 1', 'modular'),
                        'pricing2' => __('pricing table style 2', 'modular'),
                        'tabular' => __('Use the table to display tabular data', 'modular'),
                    ),
                    'value' => 'pricing',
                    'attr' => array(
                        'data-allowed-rows' => json_encode(array(
                            'pricing' => 'default-row heading-row pricing-row button-row switch-row',
                            'pricing2' => 'default-row heading-row pricing-row button-row switch-row',
                            'tabular' => 'default-row heading-row'
                                )
                        ),
                        'data-allowed-cols' => json_encode(array(
                            'pricing' => 'default-col highlight-col table-first table-last desc-col',
                            'pricing2' => 'default-col highlight-col desc-col',
            ...
            'row_options' => array(
                'name' => array(
                    ...
                    'desc' => false,
                    'choices' => array(
                        'default-row' => __('Default row', 'modular'),
                        'heading-row' => __('Heading row', 'modular'),
                        'pricing-row' => __('Pricing row', 'modular'),
                        'button-row' => __('Button row', 'modular'),
                        'switch-row' => __('Modal button row', 'modular')
                    ),
                )
            ),
            'columns_options' => array(
                'name' => array(
                    ...
                    'desc' => false,
                    'choices' => array(
                        'default-col' => __('Default column', 'modular'),
                        'highlight-col' => __('Highlight column', 'modular'),
                        'table-first' => __('Left Border Radius', 'modular'),
                        'table-last' => __('Right Border Radius', 'modular'),
                    ),
                )
            ),
            'content_options' => array(
                ...
                'switch-row' => array(
                    'button_text' => array(
                        ...
                        'desc' => false,
                        'attr' => array(
                            'placeholder' => __('button text', 'modular')
                        ),
                    ),
                ),
                'heading-row' => array(
                    ...
                    'sub_heading' => array(
                        ...
                        'desc' => false,
                        'attr' => array(
                            'placeholder' => __('sub title (can be left blank)', 'modular')
                        ),
                    ),
                ),
                'pricing-row' => array(
                    ...
                    'description' => array(
                        ...
                        'value' => '',
                        'attr' => array(
                            'placeholder' => __('per day', 'modular')
                        ),
                        'wrapper_attr' => array(
                            'class' => 'fw-col-sm-6'
                        )
                ...
                'button-row' => array(
                    'button' => ($button = fw()->extensions->get('shortcodes')->get_shortcode('button')) ? array(
                'type' => 'popup',
                'popup-title' => __('Button', 'modular'),
                'button' => __('Add', 'modular'),
